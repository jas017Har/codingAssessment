...
jsonData = [{"id": 1, "president": 1, "nm": "George Washington", "pp": "None, Federalist", "tm": "1789-1797"},
            {"id": 2, "president": 2, "nm": "John Adams", "pp": "Federalist", "tm": "1797-1801"},
            {"id": 3, "president": 3, "nm": "Thomas Jefferson", "pp": "Democratic-Republican", "tm": "1801-1809"},
            {"id": 4, "president": 4, "nm": "James Madison", "pp": "Democratic-Republican", "tm": "1809-1817"},
            ...
            {"id": 45, "president": 45, "nm": "Donald Trump", "pp": "Republican", "tm": "2017-"}]
...
def download_file():
    ...
    # applying filter for excluding Federalist party
    df2 = df1[~df1["pp"].str.contains('Federalist')]
    # Sorting the presidents in alphabetical order
    df3 = df2.sort_values('nm')
    df3[['First', 'Middle', 'Last', 'Another']] = df3.nm.str.split(" ", expand=True, )
    df3.drop(columns=['Middle', 'Last', 'Another', 'nm'], inplace=True)
    # Reversing the Name of the President
    df3['Name'] = df3.loc[:, 'First'].apply(lambda x: x[::-1])
    df3.sort_values('Name')
    # Extracting the Acronym for the parties
    df3['Party'] = df1['pp'].replace(regex=[r"[a-z-' ']"], value='')
    df3['Presidential term'] = df3['tm'].str[:4]
    ...
    csv_output = output_file.getvalue()
    output_file.close()
